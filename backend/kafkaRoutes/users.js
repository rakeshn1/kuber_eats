...
router.post('/orders', checkAuth, async (req, res) => {
  try {
    req.body.path = 'viewUserOrders';
    kafka.make_request('userOrder', req.body, (error, result) => {
      if (result?.status === 200) {
        return res.status(200).json(result.data);
      } else {
        return res.status(400).json(result?.data);
...
router.put('/updateFavorites', async (req, res) => {
  try {
    req.body.path = 'updateUserFavorites';
    kafka.make_request('userOrder', req.body, (error, result) => {
      if (result?.status === 200) {
        return res.status(200).json(result.data);
      } else {
